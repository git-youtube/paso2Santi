...
services = {
  "nginx"   => { :ip => "#{RED}.10", :ip2 => "10.0.0.10", :provision=>$nginxserver,  :port=> "8080" },
  "apache1" => { :ip => "#{RED}.11", :ip2 => "10.0.0.11",  :provision=>$apacheserver, :port=> "8081" },
  "apache2" => { :ip => "#{RED}.12", :ip2 => "10.0.0.12",  :provision=>$apacheserver, :port=> "8082" },
}
...
Vagrant.configure("2") do |config|
  ...
  config.vm.define :dns do |guest|
    ...
    guest.vm.provision "shell", name: "dns-server", path: "enable-bind-9.sh", args: "#{DNSIP} #{DOMAIN}"
  end
